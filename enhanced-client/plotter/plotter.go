...
func main() {
	...
	// Settings parsing
	fmt.Println("Opening settings file")
	file, err := ioutil.ReadFile(os.Args[1])
	if err != nil {
		log.Fatal("*** ERROR opening settings file:", err)
	} else {
		fmt.Println("Settings file successfully opened")
	}
	...
	if err != nil {
		...
		fmt.Println("Settings successfully read")
	}
	...
	fmt.Println("Creating README file")
	readme, err := os.Create(settings.ExecDir + PlotDirName + "README.txt")
	if err != nil {
		log.Fatal("Cannot create README file:", err)
	} else {
		fmt.Println("README file successfully created")
	}
	...
	if settings.TcpdumpEnabled {
		...
		fmt.Println("Tcpdump was not enabled, so it will generate no tcpPlots")
	}
